...
3. [ ] **R2 — Investigator recipe (config).** Encode the 7-step process as a recipe template over the primitives, with one domain's expert worked-examples. Validate on a real case (e.g. a control investigation from a company catalog). **Acceptance criterion (gate, not a footnote) — the consistency model, not just "edges persist":** today edges are derived from `parentId` only, so a reloaded reasoning graph drops its semantic edges. "Edges persist" alone is rejected as ambiguous — it leaves whether `grounds` is *stored* vs *derived* undecided, which is the exact asymmetry R1's provenance rule had to dodge. R2 must resolve persistence **per edge kind**:
   ...
   **Status — engine + persistence landed; real grounding pending:**
   ...
   - [ ] **R2.2.0-fidelity — claim ⊆ source check (mock).** Verify the fact's claim is entailed by / supported by the cited source, not merely that the ref resolves. With a negative control (a fact whose claim isn't in its cited source must fail). Build against the mock, calibrated by what the grounded live smoke shows about how often the model launders. This is the piece that turns "resolves" into "auditable".
   - [ ] **R2.2.1 — real grounding + worked-examples.** Swap the mock `IFactSource` for an MCP-backed control/bowtie catalog + operational APIs (real `source_ref`s); validate on a real CMP/Collahuasi control investigation. Only meaningful once grounding means what it claims (fidelity above). Gate on the live smoke: a small model that doesn't reason soundly on a representative case won't be fixed by real data.
